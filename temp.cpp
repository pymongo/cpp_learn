...
int main() {
    ...
    printf("hello\n");
    unsigned char md5_output[16]; // [u8; 16]
    ...
    unsigned char *ret = MD5(str,64, md5_output);

    //time_t mut_timestamp;
    //time_t time_output = time(&mut_timestamp);
    //printf("%d, %d\n", mut_timestamp, time_output);
    

    for (int i=0; i<16; i++) {
        printf("%d ", md5_output[i]);
    }
    printf("\n");
    for (int i=0; i<16; i++) {
